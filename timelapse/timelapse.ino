...
int imageCount;                    // File Counter (persisted in EEPROM)
...
void createDir(fs::FS &fs, const char * path){
    ...
    if(fs.mkdir(path)){
        ...
        Serial.println("Directory creation failed or already exists");
    }
}
...
void setup() {
  ...
  // Initialize EEPROM and read persistent imageCount
  EEPROM.begin(EEPROM_SIZE);
  imageCount = EEPROM.readInt(IMAGE_COUNT_ADDR);
  // If EEPROM is empty (first time), start at 1
  if (imageCount <= 0) {
    imageCount = 1;
  }
  Serial.printf("Resuming with imageCount: %d\r\n", imageCount);
  Serial.printf("DEBUGGER: %d", DEBUG);
  ...
  // every 6 hours, take separate files
  ...
  if(camera_sign && sd_sign){
    ...
    switch (bucket) {
      case 0:
        ...
        break;
      case 1:
        sprintf(filename, "/bucket2/image%d.jpg", imageCount);
        break;
      case 2:
        sprintf(filename, "/bucket3/image%d.jpg", imageCount);
        break;
      case 3:
        sprintf(filename, "/bucket4/image%d.jpg", imageCount);
        break;
    }
